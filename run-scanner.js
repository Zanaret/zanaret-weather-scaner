...
// Weather Station Scanner - Command Line Interface
// Run this script to scan all weather stations and generate files in the Stations folder
...
async function main() {
    try {
        console.log('🌤️ Weather Station Scanner - Command Line Interface');
        console.log('==================================================');
        
        const api = new WeatherStationAPI();
        const stationsFolder = './Stations/';
        
        // Ensure Stations directory exists
        if (!fs.existsSync(stationsFolder)) {
            fs.mkdirSync(stationsFolder, { recursive: true });
            console.log('📁 Created Stations directory');
        }
        ...
        // Create summary file
        const summaryContent = `# Weather Station Scan Summary
...
*Generated by Weather Station Scanner*`;
        ...
        fs.writeFileSync(path.join(stationsFolder, 'SCAN_SUMMARY.md'), summaryContent);
        console.log('📄 Created scan summary file');
        ...
        for (const station of stations) {
            try {
                ...
                const xmlResponse = await api.getLatestXMLFile(station.code);
                ...
                const filepath = path.join(stationsFolder, filename);
                
                // Save the file
                fs.writeFileSync(filepath, fileContent, 'utf8');
                
                processedCount++;
                const progress = ((processedCount / stations.length) * 100).toFixed(1);
        ...
        const finalSummaryContent = `# Weather Station Scan Results
...
- ✅ **Successful Scans:** ${successfulScans} (${((successfulScans / stations.length) * 100).toFixed(1)}%)
- ❌ **Failed Scans:** ${failedScans} (${((failedScans / stations.length) * 100).toFixed(1)}%)

## Generated Files

Each station has its own file in the Stations folder with the format: \`StationName_StationCode.md\`

---
*Generated by Weather Station Scanner*
*Data source: Environment Canada (dd.meteo.gc.ca)*`;

        fs.writeFileSync(path.join(stationsFolder, 'SCAN_RESULTS.md'), finalSummaryContent);
        
        console.log('\n🎉 Weather Station Scan Complete!');
        console.log(`📁 Files generated in: ${stationsFolder}`);
